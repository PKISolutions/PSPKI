...
# process search routine
    ...
    while ($DbRow.Next() -ne -1) {
        ...
        if ($Row.CertificateTemplate -match "^(\d+\.){3}") {
            if (![string]::IsNullOrEmpty(([Security.Cryptography.Oid]$Row.CertificateTemplate).FriendlyName)) {
                $Row.CertificateTemplate = ([Security.Cryptography.Oid]$Row.CertificateTemplate).FriendlyName
            }
        }
